...
function handleSlotOrder(uint _quantity, uint _price, bool _direction, address _proposer) public returns (uint){
        ...
        if (slotNumberOrder == 0){
            ...
            for(uint i=0; i< orderBook.length; i++){
                    Trade waitingTrade = orderBook[i];
                    if (waitingTrade.direction!=slotDirection){
                        if (waitingTrade.quantity <= _quantity){
                            // the waiting trade which wants to sell will receive the tokens
							parent.settleToken(waitingTrade.proposer,_proposer,waitingTrade.quantity);                      
							parent.settleCash(_proposer,waitingTrade.proposer,waitingTrade.quantity,waitingTrade.price);
                            _quantity = _quantity - waitingTrade.quantity;
                            indexOrderToSuppress = i+1;
                            
                        } else {
                        // the waiting trade which wants to sell will receive the tokens
                        parent.settleToken(waitingTrade.proposer,_proposer,_quantity);
                        parent.settleCash(_proposer,waitingTrade.proposer,_quantity,waitingTrade.price);
                        orderBook[i].quantity = orderBook[i].quantity-_quantity;
                        _quantity = 0;
                    }
                    if (_quantity == 0){
    ...
    //use the keccak256 function to find back the order
        bytes32 tradeHash = keccak256(_quantity, _price, _direction, _proposer);
        
     // find the order in the order book  
        uint orderIndex = indexesMap[tradeHash];
        
    // suppress the order from the order book array
        delete orderBook[orderIndex];
    
    //give back the money
    if (direction) {
       // direction = true donc c'est un ordre d'achat
       orderBook[orderIndex].quantity = orderBook[orderIndex].quantity+_quantity;
       
    } else {
        // direction = false donc c'est un ordre de vente
        orderBook[orderIndex].quantity = orderBook[orderIndex].quantity-_quantity;
    }
    //cancell the allowance
    reallocateOrderBook(orderIndex)
